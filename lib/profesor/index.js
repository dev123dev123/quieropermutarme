...
Profesor.prototype.resetPassword = function(req, callback) {
  ...
  ProfesorModel.findOne({email: email}, function(err, prof) { 
    ...
    ProfesorModel.findOneAndUpdate(email, profesor, function(err, data){
      ...
      var message = {
        from: 'Soporte <soporte@quieropermutar.me>',
        to: email,
        subject: 'Nueva contraseña generada',
        text: util.format('Hola %s, una nueva contraseña fue generada: %s, porfavor cambiala yendo a la seccion de Mi Cuenta en el sitio web, gracias', data.nombres, newPassword)
      }
